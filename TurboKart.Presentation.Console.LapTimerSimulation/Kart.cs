...
internal class Kart
{
    ...
    public async Task Race()
    {
        while (true)
        {
            ...
            CrossedLineEventArgs e = new()
                { KartNo = KartNo, Lap = CurrentLap, LapTime = newLapTime, TotalTime = totalTime };
...
internal class CrossedLineEventArgs : EventArgs
{
    ...
    {
        return
            $"Kart No. {KartNo}\tLap {Lap}\t{LapTime.ToString(LapTimeSimulator.LapTimeFormatSpecifier)}\tTotal Time: {TotalTime.ToString(LapTimeSimulator.LapTimeFormatSpecifier)}";
    }
}