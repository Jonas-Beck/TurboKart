...
public class LapTimeSimulator
{
    ...
    public async Task SimulateRealTime(int noOfCarts, int durationInMinutes, double simSpeed, int sessionID = 1)
    {
        ...
        Parallel.Invoke(new ParallelOptions() { MaxDegreeOfParallelism = 1 },
            async () => await StartRaceClock(duration));
    ...
    private async Task StartRaceClock(TimeSpan duration)
    {
        await Console.Out.WriteLineAsync(
            $"{duration.ToString(LapTimeFormatSpecifier)} real time race started at [{DateTime.Now.ToString("HH:mm:ss")}]");
    ...
    private async Task PrintResult(List<Kart> karts)
    {
        ...
        {
            s +=
                $"{i + 1}.\tDriver No. {sorted[i].KartNo}\t{sorted[i].TotalTime.ToString(LapTimeFormatSpecifier)}\t{sorted[i].CurrentLap} Laps{Environment.NewLine}";
        }
